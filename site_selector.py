...
from PIL import Image, ImageTk  # python imaging library for working with image files
import numpy as np  # math library
import cv2 as cv  # image analysis library


# CONSTANTS: User Input:
IMG_GRAY = cv.imread('mola_1024x501.png', cv.IMREAD_GRAYSCALE)  # mola b&w image
IMG_COLOR = cv.imread('mola_color_1024x506.png')  # mola color image
RECT_WIDTH_KM = 335  # limit size of rectangle
RECT_HT_KM = 335
MAX_ELEV_LIMIT = 55  # elevation limit for landing site
NUM_CANDIDATES = 20  # number of potential landing sites
MARS_CIRCUM = 21344  # circumference of martian equator
...
class Search:
    """Read image and identify landing rectangles based on input criterea."""
